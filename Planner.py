...
class Planner:
    def __init__(self, path, domain_file='crafting.pddl', exec_path='ff'):
        self.domain_file = os.path.join(path, domain_file)
        self.problem_template = open(os.path.join(
            path, 'problem_template.pddl')).read()
    ...
    def extract_plan(lines):
        ...
        plan = list(filter(lambda x: not x.startswith(';')
                    and not len(x) == 0 and not x == 'step', plan))
...
if __name__ == '__main__':
    ...
    planner = Planner(exec_path='/home/plymper/trajectory-abstraction/pddlgym_planners/FF-v2.3/ff',
                      path='pddl', domain_file='crafting.pddl')
