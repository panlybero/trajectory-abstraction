...
def get_truth_fractions_per_group(states,groups):
    ...
    group_descriptions = {i:predicate_dict.copy() for i in range(len(set(groups)))}
    ...
    for state,group in zip(states,groups):
        ...
        group_descriptions[group] = {k:v+state_description[k] for k,v in group_descriptions[group].items()}
    
    for group in group_descriptions:
        group_descriptions[group] = {k:v/group_counts[group] for k,v in group_descriptions[group].items()}
...
def get_per_step_action_probs(describe_state,data, n_actions=7):
    ...
    per_set_action_probs = {s:np.zeros(n_actions) for s in description_to_steps}
...
def plot_model(data, generalizations, model,fname='file.dot', actions=['up', 'down', 'left', 'right', 'craft_planks', 'craft_chair_parts', 'craft_chair'], from_data=True,clustering=None):
    if from_data:
        per_set_action_probs = get_per_step_action_probs(lambda x: x, data, n_actions=len(actions))
    else:
        cluster_counts = clustering.compute_cluster_counts()
        generalizations = list(clustering.generalizations)
        per_set_action_probs = {k:cluster_counts[i]/np.sum(cluster_counts[i]) for i,k in enumerate(generalizations)}
        

    per_set_action_probs = {k:[(actions[i],f"{v:.2f}") for i,v in enumerate(per_set_action_probs[k])] for k in per_set_action_probs}
...
def get_per_generalization_data(data, generalizations):
    per_generalization_data_states = {frozenset(i):[] for i in generalizations}
    per_generalization_data_actions = {frozenset(i):[] for i in generalizations}
    ...
    for i,ep in enumerate(data):
        for d in ep:
            ...
            d_gen = frozenset(get_most_specific_generalization(generalizations,state))
    ...
    per_generalization_action_probs = {g:np.zeros(7) for g in per_generalization_data_actions}
    for g in per_generalization_data_actions:
        for a in per_generalization_data_actions[g]:
            per_generalization_action_probs[g][a] += 1
        per_generalization_action_probs[g] /= np.sum(per_generalization_action_probs[g])
...
def invent_symbol(state_description,action_dist):
    ...
    new_description2 = frozenset(state_description.union(set([f"not({new_predicate})"])))
    ...
    new_generalizations = {new_description1:action_dist1, new_description2:action_dist2}
...
def split_cluster(per_generalization_data):
    ...
    per_state_action_probs = get_per_step_action_probs(lambda state: description_to_set(describe_state(state)),[per_generalization_data])

    distances = compute_cross_entropies(per_state_action_probs)
    
    try:
        clustering = AgglomerativeClustering(n_clusters=2, affinity='precomputed', linkage='single').fit(distances)
...
def get_abstract_trajectories(data,generalizations, factory):
    ...
    for ep in data:
        new_ep = []
        for step in ep:
            state_desc = factory.create_state_description(list(description_to_set(describe_state(step[0]))))
            applicable_generalizations = state_desc.get_applicable_generalizations(generalizations)
            if applicable_generalizations is None:
                raise ValueError("No applicable generalization found")
            new_ep.append((applicable_generalizations[0],step[1])) # use most specific generalization
...
if __name__=='__main__':
    ...
    factory = StateDescriptionFactory({"is_next_to_wood", "has_wood", "has_planks", "has_chair_parts", "has_chair"})
    ...
    per_state_action_probs = get_per_step_action_probs(lambda state: factory.create_state_description(list(description_to_set(describe_state(state)))),data, n_actions=len(actions))
    
    state_descriptions = list(per_state_action_probs.keys())
    action_probability_vectors = list(per_state_action_probs.values())

    clustering = StateDescriptionClustering(state_descriptions, action_probability_vectors)
    ...
    plot_model(abstract_data, generalizations, model, actions=actions, fname='abstract_model.dot')
    ...
    abstract_data = get_abstract_trajectories(data,generalizations,factory)
    model = fit_transition_model(abstract_data,generalizations)
    plot_model(abstract_data, generalizations, model, actions=actions, fname='abstract_model1.dot')
    ...
    model = fit_transition_model(abstract_data,generalizations)
    plot_model(abstract_data, generalizations, model, actions=actions, fname='abstract_model1.dot', from_data=False, clustering=clustering)
